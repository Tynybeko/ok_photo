...
import { dedupeStateFromManifest } from './dedupe.js';
...
export async function addDeleted(id) {
  const r = await addDeletedMany([id]);
  return r;
}

export async function addDeletedMany(ids) {
  const unique = [...new Set(ids.filter(Boolean))];
  if (!unique.length) return getDeleted();
  for (let attempt = 0; attempt < 4; attempt++) {
    const deleted = await getDeleted();
    for (const id of unique) deleted.add(id);
    await saveDeleted(deleted);
    const check = await getDeleted();
    if (unique.every((id) => check.has(id))) return check;
  }
  const deleted = await getDeleted();
  for (const id of unique) deleted.add(id);
  await saveDeleted(deleted);
...
export async function getDedupeState() {
  return dedupeStateFromManifest(await getManifest());
}

/** @deprecated используй getDedupeState */
export async function existingUrlKeys() {
  const { urlKeys } = await getDedupeState();
  return urlKeys;
}

const NOTIFY_STATE_PATH = 'gallery/notify-state.json';

export async function getNotifyState() {
  return readJson(NOTIFY_STATE_PATH, { lastPeriod: -1 });
}

export async function saveNotifyState(data) {
  await writeJson(NOTIFY_STATE_PATH, data);
}
